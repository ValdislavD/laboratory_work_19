...
void read_polynomials(const char* filename, PolynomialTerm** polynomials, int* counts, int* num_polynomials) {
    ...
    *num_polynomials = 0;
    while (fread(&counts[*num_polynomials], sizeof(int), 1, file) == 1) {
        polynomials[*num_polynomials] = (PolynomialTerm*)malloc(counts[*num_polynomials] * sizeof(PolynomialTerm));
        fread(polynomials[*num_polynomials], sizeof(PolynomialTerm), counts[*num_polynomials], file);
        (*num_polynomials)++;
    }
...
void test_process_file_s() {
    ...
    PolynomialTerm poly1[] = {{2, 3.0}, {1, -2.0}, {0, 1.0}}; // 3x^2 - 2x + 1 (не корень для x=1)
    PolynomialTerm poly2[] = {{2, 1.0}, {0, -1.0}};           // x^2 - 1 (корень для x=1)
    PolynomialTerm poly3[] = {{1, 1.0}, {0, -1.0}};           // x - 1 (корень для x=1)
    ...
    read_polynomials(output_filename, out_polynomials, out_counts, &out_num_polynomials);
